/* Science course records are additive and separate from every baseline assessment key. */
(() => {
 'use strict';
 const C=window.SCIENCE_COURSE,prefix='roryScienceBridge2026_',parentKey='roryScienceParent2026',recordKey='roryHomeschoolRecordV2';
 const weeks=()=>C.weeks;
 ...
 function validateWork(id,value){
  const week=weeks().find(x=>x.id===id);if(!week)throw Error('Unknown Science week.');
  const ids=fieldIds(week),checks=checkIds(week),object=x=>x&&typeof x==='object'&&!Array.isArray(x);
  if(!object(value)||value.schemaVersion!==1||!object(value.fields)||!object(value.taskChecks)||!Array.isArray(value.snapshots)||typeof value.complete!=='boolean'||(value.updatedAt!==null&&!date(value.updatedAt))||(value.completedAt!==null&&!date(value.completedAt)))throw Error('Science work has an unsupported format. Existing data was preserved.');
  if(Object.entries(value.fields).some(([k,v])=>!ids.has(k)||typeof v!=='string'))throw Error('Science response fields need recovery.');
  if(Object.entries(value.taskChecks).some(([k,v])=>!checks.has(k)||typeof v!=='boolean'))throw Error('Science activity checks need recovery.');
  const snapshotIds=new Set();for(const s of value.snapshots){if(!s||typeof s.id!=='string'||!s.id||snapshotIds.has(s.id)||!date(s.at)||!s.fields||!s.taskChecks)throw Error('Invalid Science checkpoint.');snapshotIds.add(s.id);}
  if(value.complete&&(!value.completedAt||!value.snapshots.length))throw Error('Completed Science work is missing its checkpoint.');
 ...
 function parent(){const value=read(parentKey,{schemaVersion:1,weeks:{},summary:''});if(!value||value.schemaVersion!==1||!value.weeks||typeof value.weeks!=='object'||Array.isArray(value.weeks)||typeof value.summary!=='string')throw Error('Science parent records need recovery; nothing was overwritten.');return value;}
 ...
 function restorePlan(incoming){
  if(!incoming)return [];
  if(incoming.schemaVersion!==1||incoming.courseVersion!==C.version||!incoming.weekRecords||Array.isArray(incoming.weekRecords))throw Error('Choose a supported Science course export.');
  const valid=new Set(weeks().map(w=>w.id)),plans=[];
  for(const [id,value] of Object.entries(incoming.weekRecords)){
   if(!valid.has(id))throw Error('Unknown Science week in backup.');validateWork(id,value);
  ...
  if(incoming.parentReview){const p=incoming.parentReview;if(!p||p.schemaVersion!==1||!p.weeks||typeof p.summary!=='string')throw Error('Invalid Science parent review.');const current=parent();plans.push([parentKey,{...p,...current,weeks:{...p.weeks,...current.weeks},summary:current.summary||p.summary}]);}
  return plans;
 }
 ...
 function syncPortal(){
  ...
  if(!['assignments','assessments','logs','portfolio'].every(k=>Array.isArray(record[k])))throw Error('The school record needs recovery. Science work was kept separate.');
  ...
  for(const week of weeks()){
   ...
   if(!record.portfolio.some(x=>x.id===week.id)){record.portfolio.push({id:week.id,title:'Science Week '+week.week+' · '+week.title,subject:'Science',date:value.completedAt.slice(0,10),description:'Saved investigation data, evidence, claim, and end check. Portfolio work: '+week.portfolio+' Completion is not a mastery grade. Standards: '+week.standards.join(', '),link:'science.html?id='+week.id});changed=true;}
  }
  if(changed)write(recordKey,record);return record;
 }
 window.ScienceStore={C,prefix,parentKey,recordKey,weeks,commonFields,allFields,fieldIds,checkIds,read,write,esc,blank,validateWork,work,save,evidence,parent,download,backup,restorePlan,restore,syncPortal};
})();
